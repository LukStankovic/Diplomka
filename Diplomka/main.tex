...
% Bude napsáno o tom jak můžou metriky vypadat (tabulka, graf, ...) a jak je lze vizualizovat (birt + odkaz na praktickou část). TODO LS
...
Z výsledků analýzy můžeme vidět, že nejrychlejší algoritmus pro přečtení a zpracování jednoho artefaktu je nejrychlejší algoritmus číslo 4, který používá pouze cykly a objekty \textit{SimpleXMLElement}. Druhý nejrychlejší byl na základě analýzy algoritmus 2, který kombinuje technologii XPath a objekty \textit{SimpleXMLElement}. Poté následují nejpomalejší algoritmy 3 a 1.

Nejrychlejší algoritmus 4 má výhodu kromě rychlosti také v možnosti používat metody pro přístup k atributům daného objektu. Bohužel je zde velká nevýhoda ve formě složitosti a délce celého kódu, který je navíc kvůli velkého počtu cyklů pro vnořené elementy velmi nepřehledný. Zároveň dle dalších testů je velmi pravděpodobné, že rychlost se zpomalí při čtení více artefaktů za sebou, jelikož se zaplní paměť velkým množstvím objektů \textit{SimpleXMLElement}.

Z tohoto důvodu byl pro implementaci extrakci a zpracování dat artefaktů z XML zvolen algoritmus 2, který kombinuje technologii XPath a objekty typu \textit{SimpleXMLElement}.




Stahovaná data z platformy IBM Jazz se ukládají do NoSQL MongoDB databáze.
...
Pakliže si přeje uživatel vytvořit metriku úplně od základu bude nejprve potřebovat vytvořit definici dané metriky.
