...
\appendix
\chapter{Adresářová struktura nástroje}


\dirtree{%
.1 /.
.2 assets \DTcomment{Kaskádové styly a JavaScript}.
.2 bin \DTcomment{Symfony konzole a další nástroje}.
.2 config \DTcomment{Konfigurační soubory}.
.2 docker \DTcomment{Konfigurační soubory pro Docker}.
.2 docs \DTcomment{Dokumentace a ukázkové soubory}.
.2 node\_modules \DTcomment{JavaScript závislosti (stažené pomocí \texttt{npm})}.
.2 public \DTcomment{Veřejná složka, například pro nahrávání souborů}.
.2 src \DTcomment{Hlavní adresář se zdrojovými kódy v jazyku PHP}.
.3 Command \DTcomment{Soubory spustitelné z terminálu}. 
.3 Component \DTcomment{Komponenty pracující s externími službami}.
.3 DataFixtures \DTcomment{Ukázková data}.
.3 Document \DTcomment{Dokumenty a logika pro MongoDB}.
.3 Form \DTcomment{Definice formulářů}.
.3 Migrations \DTcomment{SQL příkazy pro definování SQL databáze}.
.3 Model \DTcomment{Entity pro SQL databázi}.
.3 Security \DTcomment{Přihlašování do nástroje}.
.2 templates \DTcomment{Šablony pro frontend}.
.2 tests \DTcomment{Automatické testy}.
.2 translations \DTcomment{Překlady pro veškeré texty}.
.2 vendor \DTcomment{PHP závislosti (stažené pomocí nástroje Composer)}.
.2 var \DTcomment{Logy a mezipaměť}.
.2 web \DTcomment{Styly a skripty třetích stran}.
}
...
dng_rm_project_uri="_CKGS4P3GEeqMt4CHFFzMsw"
\end{lstlisting}
