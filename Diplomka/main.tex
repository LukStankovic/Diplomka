...
.3 UX \DTcomment{Obrazovky uživatelského rozhraní}.
.3 readme.txt \DTcomment{Návod s praktickými informacemi}.
}

\chapter{Instalace prototypového nástroje}
Nástroj je možné nainstalovat jak pro vývoj, tak pro produkční prostředí pomocí dvou možností. Pomocí obou možností lze nástroj nainstalovat na všech třech základních operačních systémů -- Windows, Linux a macOS. Obě možnosti taktéž nabízí výhody a nevýhody, které je nutné při instalaci zvážit.
...
Jakmile je nainstalovaný podsystém Debian, můžeme zde nainstalovat Docker pro Linux, nástroj \textit{docker-compose} a \textit{Composer}:
\begin{enumerate}
...
\end{figure}
\section{Artefakty}
V uživatelské sekci si může uživatel zobrazit například veškeré artefakty, které byly dle jednotlivých projektů staženy z platformy IBM Jazz. Seznam artefaktů je možné nalézt na obrázku \ref{fig:metrics_tool_artifacts}. Pomocí formuláře, lze také mezi jednotlivými artefakty filtrovat dle projektu.
